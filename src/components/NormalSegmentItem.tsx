...
const NormalSegmentItem = (props: {
  ...
  const { item, idx, isIn, moveCallback, move2Callback, segmentStartIdx } = props
  ...
  const transText = useMemo(
    () => getTransText(transResult, envData.hideOnDisableAutoTranslate, autoTranslate),
    [autoTranslate, envData.hideOnDisableAutoTranslate, transResult]
  )
  const display = useMemo(
    () => getDisplay(envData.transDisplay, item.content, transText),
    [envData.transDisplay, item.content, transText]
  )
  ...
  return (
    <div
      className={classNames(
        'flex py-0.5 cursor-pointer rounded-sm hover:bg-base-200 group',
        fontSize === 'large' ? 'text-sm' : 'text-xs'
      )}
      onClick={moveCallback}
      onDoubleClick={move2Callback}
    >
      <div className='desc w-[66px] flex justify-center items-center'>
        {formatTime(item.from)}
      </div>
      <div className='flex-1 min-w-0'>
        <div className={classNames('font-medium', isIn ? 'text-primary underline' : '')}>
          {displayMain}
        </div>
        {displaySub && <div className='desc'>{displaySub}</div>}
      </div>
      <div className='flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity'>
        <button
          className={classNames(
            'btn btn-ghost btn-xs btn-circle p-0.5',
            isBookmarked ? 'text-primary' : 'text-base-content/50'
          )}
